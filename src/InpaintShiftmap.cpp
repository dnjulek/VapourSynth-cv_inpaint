...
struct SHIFTMAPData final {
	VSNode* node;
	VSNode* node2;
	const VSVideoInfo* vi;
};

static void shiftmap_process_gray(const VSFrame* src, const VSFrame* src2, VSFrame* dst, const SHIFTMAPData* const VS_RESTRICT d, const VSAPI* vsapi) noexcept {
	...
	cv::xphoto::inpaint(srcImg, maskImg, dstImg, cv::xphoto::INPAINT_SHIFTMAP);
...
static void shiftmap_process_rgb(const VSFrame* src, const VSFrame* src2, VSFrame* dst, const SHIFTMAPData* const VS_RESTRICT d, const VSAPI* vsapi) noexcept {
	const int w = vsapi->getFrameWidth(src, 0);
	const int h = vsapi->getFrameHeight(src, 0);
	ptrdiff_t stride = vsapi->getStride(src, 0);
	...
	cv::Mat srcImg_Lab(cv::Size(w, h), CV_8UC3);
	cv::Mat dstImg_Lab(cv::Size(w, h), CV_8UC3);
	...
	cv::cvtColor(srcImg_bgr, srcImg_Lab, cv::COLOR_BGR2Lab);
	cv::xphoto::inpaint(srcImg_Lab, maskImg, dstImg_Lab, cv::xphoto::INPAINT_SHIFTMAP);
	cv::cvtColor(dstImg_Lab, dstImg_bgr, cv::COLOR_Lab2BGR);
...
static const VSFrame* VS_CC shiftmapGetFrame(int n, int activationReason, void* instanceData, void** frameData, VSFrameContext* frameCtx, VSCore* core, const VSAPI* vsapi) {
	auto d{ static_cast<SHIFTMAPData*>(instanceData) };
	...
	if (activationReason == arInitial) {
		...
		if (d->vi->format.colorFamily == cfGray) {
			shiftmap_process_gray(src, src2, dst, d, vsapi);
		}
		else {
			shiftmap_process_rgb(src, src2, dst, d, vsapi);
		}
...
static void VS_CC shiftmapFree(void* instanceData, VSCore* core, const VSAPI* vsapi) {
	auto d{ static_cast<SHIFTMAPData*>(instanceData) };
...
void VS_CC shiftmapCreate(const VSMap* in, VSMap* out, void* userData, VSCore* core, const VSAPI* vsapi) {
	auto d{ std::make_unique<SHIFTMAPData>() };
	int err{ 0 };
	...
	if (d->vi->format.bytesPerSample != 1 ||
		...
		vsapi->mapSetError(out, "InpaintShiftmap: only Gray8 and RGB24 formats supported.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	if (vi2->format.bytesPerSample != 1) {
		vsapi->mapSetError(out, "InpaintShiftmap: mask clip should be in 8-bit format.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	if (d->vi->width != vi2->width ||
		...
		vsapi->mapSetError(out, "InpaintShiftmap: main clip and mask clip must have the same size.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	if (d->vi->numFrames != vi2->numFrames) {
		vsapi->mapSetError(out, "InpaintShiftmap: main clip and mask clip must have the same number of frames.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	vsapi->createVideoFilter(out, "InpaintShiftmap", d->vi, shiftmapGetFrame, shiftmapFree, fmParallel, deps, 2, d.get(), core);
	d.release();
}