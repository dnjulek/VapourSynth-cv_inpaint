...
static void ns_process_gray(const VSFrame* src, const VSFrame* src2, VSFrame* dst, const NSData* const VS_RESTRICT d, const VSAPI* vsapi) noexcept {
	...
	cv::inpaint(srcImg, maskImg, dstImg, d->radius, cv::INPAINT_NS);
...
static void ns_process_rgb(const VSFrame* src, const VSFrame* src2, VSFrame* dst, const NSData* const VS_RESTRICT d, const VSAPI* vsapi) noexcept {
	const int w = vsapi->getFrameWidth(src, 0);
	const int h = vsapi->getFrameHeight(src, 0);
	ptrdiff_t stride = vsapi->getStride(src, 0);
	...
	cv::inpaint(srcImg_bgr, maskImg, dstImg_bgr, d->radius, cv::INPAINT_NS);
...
static const VSFrame* VS_CC nsGetFrame(int n, int activationReason, void* instanceData, void** frameData, VSFrameContext* frameCtx, VSCore* core, const VSAPI* vsapi) {
	auto d{ static_cast<NSData*>(instanceData) };
	...
	if (activationReason == arInitial) {
		...
		if (d->vi->format.colorFamily == cfGray) {
			ns_process_gray(src, src2, dst, d, vsapi);
		}
		else {
			ns_process_rgb(src, src2, dst, d, vsapi);
		}
...
static void VS_CC nsFree(void* instanceData, VSCore* core, const VSAPI* vsapi) {
	auto d{ static_cast<NSData*>(instanceData) };
...
void VS_CC nsCreate(const VSMap* in, VSMap* out, void* userData, VSCore* core, const VSAPI* vsapi) {
	auto d{ std::make_unique<NSData>() };
	int err{ 0 };
	...
	if (d->vi->format.bytesPerSample != 1 ||
		...
		vsapi->mapSetError(out, "InpaintNS: only Gray8 and RGB24 formats supported.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	if (vi2->format.bytesPerSample != 1) {
		vsapi->mapSetError(out, "InpaintNS: mask clip should be in 8-bit format.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	if (d->vi->width != vi2->width ||
		...
		vsapi->mapSetError(out, "InpaintNS: main clip and mask clip must have the same size.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	if (d->vi->numFrames != vi2->numFrames) {
		vsapi->mapSetError(out, "InpaintNS: main clip and mask clip must have the same number of frames.");
		vsapi->freeNode(d->node);
		vsapi->freeNode(d->node2);
		return;
	}
	...
	vsapi->createVideoFilter(out, "InpaintNS", d->vi, nsGetFrame, nsFree, fmParallel, deps, 2, d.get(), core);
	d.release();
}